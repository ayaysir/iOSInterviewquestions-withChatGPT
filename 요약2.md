...
- **Method Swizzling**은 실행 중에 클래스의 메서드 구현을 동적으로 교체하는 기술입니다. 이를 통해 기존 메서드의 동작을 수정하거나 확장할 수 있습니다. 주로 디버깅, 모니터링, 성능 측정 또는 AOP(Aspect-Oriented Programming)와 같은 목적으로 사용됩니다.

### NSCoder 클래스는 어떤 상황에서 어떻게 써야 하는지 설명하시오.
- **NSCoder** 클래스는 객체를 데이터 스트림으로 인코딩(직렬화)하거나 디코딩(역직렬화)할 때 사용됩니다. 주로 파일에 데이터를 저장하거나 네트워크를 통해 객체를 전송할 때, 객체의 상태를 저장하고 복원하기 위해 활용됩니다.

### Responder Chain 구조에 대해 설명하고, First Responder 역할에 대해 설명하시오.
- **Responder Chain**은 iOS 및 macOS에서 이벤트 처리를 위한 계층적인 구조를 나타냅니다. 이벤트가 발생하면 Responder Chain을 따라 이벤트를 처리할 객체를 찾아가게 됩니다. 이를 통해 이벤트에 따라 여러 객체가 연쇄적으로 처리할 수 있습니다.
- **First Responder**는 현재 이벤트를 처리할 수 있는 최상위 객체를 의미합니다. 주로 사용자 입력을 받는 컨트롤이나 뷰에서 사용되며, Responder Chain에서 이벤트를 먼저 수신하고 처리합니다.

### NSObject부터 UIButton 까지 상속 과정의 계층과 역할을 설명하시오.
- **NSObject**: 모든 Objective-C 객체의 기본 클래스로, 메모리 관리와 런타임 시스템의 기본 동작을 제공합니다.
- **UIResponder**: 사용자 이벤트를 처리하기 위한 기본 클래스. iOS 애플리케이션의 View와 Controller에 사용됩니다.
- **UIView**: 화면에 표시되는 모든 UI 요소의 기본 클래스로, 사용자 인터페이스를 구성하고 이벤트를 처리합니다.
- **UIControl**: 사용자 상호작용을 위한 컨트롤 요소의 기본 클래스로, 버튼(Button), 스위치(Switch) 등을 포함합니다.
- **UIButton**: 사용자가 탭하거나 클릭할 수 있는 버튼을 나타내는 클래스입니다.

### shallow copy와 deep copy의 차이점을 설명하시오.
- **Shallow Copy**: 복사 대상 객체를 새로운 객체에 복사할 때, 객체의 주소값만 복사하여 참조하는 방식입니다. 복사된 객체와 원본 객체는 같은 객체를 참조하고 있으므로 내부 객체의 변경은 영향을 받습니다.
- **Deep Copy**: 복사 대상 객체를 새로운 객체에 복사할 때, 객체와 내부 객체까지 모두 새로운 인스턴스로 복사하는 방식입니다. 복사된 객체는 원본과 독립적으로 동작하며, 내부 객체의 변경이 원본 객체에 영향을 주지 않습니다.

### Push Notification 방식에 대해 설명하시오.
- **Push Notification**은 서버에서 클라이언트로 메시지를 보내는 푸시 알림 방식입니다. 애플리케이션을 실행하지 않은 상태에서도 알림을 전송할 수 있으며, 주로 원격 서버에서 알림을 발송하여 사용자에게 알려줍니다.

### Foundation 과 Core Foundation 프레임워크의 차이점을 설명하시오.
- **Foundation 프레임워크**: Objective-C와 Swift에서 사용할 수 있는 고수준 프레임워크로, 문자열 처리, 컬렉션, 네트워킹, 파일 처리 등의 기능을 제공합니다.
- **Core Foundation 프레임워크**: C 언어 기반의 저수준 프레임워크로, Foundation의 일부 기능을 C API로 제공하며, 메모리 관리, 데이터 타입, 컬렉션, 네트워킹 등의 기능을 제공합니다.

### NSURLConnection 에서 사용하는 Delegate 메서드들에 대해 설명하시오.
- **NSURLConnection Delegate 메서드**:
  - `connection:didReceiveResponse:`: 서버로부터 응답을 받았을 때 호출됩니다.
  - `connection:didReceiveData:`: 서버로부터 데이터를 받았을 때 호출됩니다.
  - `connectionDidFinishLoading:`: 데이터 수신이 완료되었을 때 호출됩니다.
  - `connection:didFailWithError:`: 데이터 수신 중 에러가 발생했을 때 호출됩니다.

### Synchronous 방식과 Asynchronous 방식으로 URL Connection을 처리할 경우의 장단점을 비교하시오.
- **Synchronous(동기적) 방식**:
  - 장점: 코드가 간단하고 직관적입니다.
  - 단점: 요청이 블로킹되어 UI가 응답하지 않는 문제가 발생할 수 있습니다.

- **Asynchronous(비동기적) 방식**:
  - 장점: UI 블로킹을 피하고 다중 작업을 처리할 수 있습니다.
  - 단점: 콜백 형태로 처리되므로 코드가 복잡해질 수 있습니다.

### Plist 파일 구조와 Plist 파일에 저장된 데이터를 다루기 적합한 클래스를 설명하시오.
- **Plist 파일 구조**: 프로퍼티 리스트(Property List) 형식으로 데이터를 저장하는 파일입니다. XML 또는 이진 형태로 저장됩니다.
- **다루기 적합한 클래스**: Foundation 프레임워크의 `NSDictionary` 또는 `NSArray` 클래스를 사용하여 Plist 파일의 데이터를 읽고 쓸 수 있습니다.

### Core Data와 SQLite 같은 데이터베이스의 차이점을 설명하시오.
- **Core Data**:
  - 객체 그래프 관리를 위한 프레임워크로, 데이터를 객체로 다룹니다.
  - 데이터를 SQLite, XML, 이진 파일 등에 저장할 수 있습니다.
  
- **SQLite**:
  - 관계형 데이터베이스로, SQL을 사용하여 데이터를 관리합니다.
  - 테이블과 스키마를 정의하고, SQL 쿼리를 사용하여 데이터를 조작합니다.

### JSON 데이터를 처리하는 방식과 파서, 객체 변환 방식에 대해 설명하시오.
- **JSON 데이터 처리 방식**:
  - `JSONSerialization`: Foundation 프레임워크에서 제공하는 JSON 데이터의 인코딩 및 디코딩 기능을 제공합니다.
  - `Codable 프로토콜`: Swift에서 JSON 데이터와 객체 간의 변환을 지원합니다.

### 웹 서버와 HTTP 연결을 사용해서 데이터를 주거나 받으려면 사용해야 하는 클래스와 동작을 설명하시오.
- `URLSession` 클래스와 관련된 클래스 및 메서드를 사용하여 HTTP 요청을 만들고, 응답을 받아 처리할 수 있습니다. `URLRequest`, `URLResponse`, `URLSessionDataTask` 등의 클래스를 사용하여 데이터를 주고받을 수 있습니다.

### Protocol에서는 왜 var만 되는지 설명하시오.
- Protocol에서 선언된 프로퍼티는 프로토콜의 일반적인 형태를 나타내기 때문에 저장 프로퍼티만 선언할 수 있습니다. 프로토콜은 구현을 포함하지 않으므로, 연산 프로퍼티만 정의할 수 있습니다.

### DispatchQueue.main.sync를 사용하는 상황을 설명하시오.
- `DispatchQueue.main.sync`는 메인 큐에서 동기적으로 작업을 실행하는 방식으로, 주로 UI 업데이트와 관련된 작업을 메인 쓰레드에서 처리할 때 사용합니다. 이 작업은 메인 큐에서 실행되므로 메인 큐가 블록되지 않도록 주의해야 합니다.

#### iOS 프로그래밍에서 UI 관련 메서드들이 메인 스레드에서 실행되어야 하는 이유
- 앱의 Responsiveness 보장: 사용자 인터페이스는 사용자의 입력에 신속하게 반응해야 합니다. 메인 스레드에서 UI 작업을 처리하면 UI의 레이어와 사용자 입력 간의 신속한 상호작용을 보장할 수 있습니다.
- 동시성과 Race Condition 방지: 여러 스레드에서 동시에 UI를 업데이트하면 두 스레드가 동일한 UI 요소를 수정할 수 있습니다. 이는 경쟁 상태(Race Condition)를 초래할 수 있어서 예측 불가능한 버그를 발생시킬 수 있습니다. 메인 스레드에서만 UI 업데이트를 수행하면 이러한 문제를 방지할 수 있습니다.

### Run Loops에 대해 설명하시오.
- **Run Loop**은 이벤트 처리, 타이머 관리, 입출력 관리 등을 담당하는 이벤트 처리 루프입니다. 애플리케이션의 이벤트를 받아서 처리하고, 작업을 관리하여 애플리케이션의 동작을 지속적으로 유지합니다.RunLoop은 이벤트를 처리하는 주기적인 루프로, 여러 모드(Mode)에서 동작할 수 있습니다.
